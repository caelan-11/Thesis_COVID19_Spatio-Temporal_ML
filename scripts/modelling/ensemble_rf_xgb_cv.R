...
dir.create("../Figures/ensemble_cv/RF_XGB/", recursive = TRUE, showWarnings = FALSE)
dir.create("../Outputs/ensemble_cv/RF_XGB/", recursive = TRUE, showWarnings = FALSE)
...
# Cross-validated stacking (RF + XGB as base learners)
run_fold <- function(k) {
  cat("Running Fold", k, "\n")
  ...
  set.seed(100 + k) 
  rf_fit <- randomForest(
    logRNA ~ ., data = train[, c("logRNA", covariates)],
    ntree = 750, mtry = 5
  )
  ...
  xgb_pred_train <- predict(xgb_fit, newdata = x_train)
  xgb_pred_test  <- predict(xgb_fit, newdata = x_test)
  ...
  meta_fit <- lm(logRNA ~ rf_pred + xgb_pred, data = meta_train)
...
readr::write_csv(ensemble_metrics_by_fold, "../Outputs/ensemble_cv/RF_XGB/optimised_ensemble_rf_xgb_metrics_by_fold.csv")
...
readr::write_csv(overall_ensemble_metrics, "../Outputs/ensemble_cv/RF_XGB/optimised_ensemble_rf_xgb_metrics_overall.csv")
...
# Save to .csv
readr::write_csv(data_oos, "../Outputs/ensemble_cv/RF_XGB/data_oos_optimised.csv")
